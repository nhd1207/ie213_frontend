...
function* getListCarByIDSaga(action) {
    try {
        ...
        if (response.status === "success") {
            yield all([
                put({ type: TYPE.GETCARORDER.SUCCESS, ...response }),
            ])
        } else {
        ...
        yield all([
            put({ type: TYPE.GETCARORDER.ERROR, error })
        ])
    }
}
...
function* getListShowroomSaga(action) {
    try {
        ...
        if (response.status === "success") {
            yield all([
                put({ type: TYPE.GETLISTSHOWROOM.SUCCESS, ...response }),
            ])
        } else {
        ...
        yield all([
            put({ type: TYPE.GETLISTSHOWROOM.ERROR, error })
        ])
    }
}
...
function* createCarOrderSaga(action) {
    try {
        ...
        yield all([
            put({ type: TYPE.CREATECARORDER.ERROR, error })
        ])
    }
}
